...
class TestInfrastructureFailureTagging:
    """Test that infrastructure failures (SIGTERM, reconciliation) don't burn retry budget."""

    def test_sigterm_failure_has_infrastructure_flag(
        self, mutation_log, work_state, audit_log, session_logger, tmp_project
    ):
        """SIGTERM (exit_code 143) includes infrastructure: True in mutation data."""
        _create_task(mutation_log, "t1", "Task 1")
        mutation_log.append("task_started", {"attempt": 1}, reason="test", task_id="t1")
        work_state.refresh()
        task = work_state.get_task("t1")

        result = AgentResult(output="Killed", exit_code=143, duration_s=1.0)
        process_completed(
            task=task,
            result=result,
            attempt=1,
    ...
    def test_sigterm_does_not_increment_attempt_count(
        self, mutation_log, work_state, audit_log, session_logger, tmp_project
    ):
        """SIGTERM failure does not increment attempt_count (preserves retry budget)."""
        _create_task(mutation_log, "t1", "Task 1")
        mutation_log.append("task_started", {"attempt": 1}, reason="test", task_id="t1")
        work_state.refresh()
        task = work_state.get_task("t1")
        ...
        task = work_state.get_task("t1")
        assert task["status"] == "failed"
        assert task["attempt_count"] == 0  # NOT incremented
    ...
    def test_normal_failure_still_increments_attempt_count(
        ...
        # Check mutation has no infrastructure flag
        entries = mutation_log.read_all()
        fail_entries = [e for e in entries if e["type"] == "task_failed"]
        assert len(fail_entries) == 1
        ...
        task = work_state.get_task("t1")
        assert task["status"] == "failed"
        assert task["attempt_count"] == 1  # incremented normally

    def test_infrastructure_failures_dont_burn_retry_budget(
        self, mutation_log, work_state, audit_log, session_logger, tmp_project
    ):
        """Three infrastructure deaths should not escalate — retry budget preserved."""
        _create_task(mutation_log, "t1", "Task 1", max_retries=3)

        # Simulate 3 SIGTERM failures
        for i in range(1, 4):
            mutation_log.append(
                "task_started", {"attempt": i}, reason="test", task_id="t1"
            )
            work_state.refresh()
            task = work_state.get_task("t1")
        ...
        task = work_state.get_task("t1")
        assert task["attempt_count"] == 0
        assert task["status"] == "failed"  # NOT escalated

        # Task should still be in ready tasks (retries not exhausted)
        ready = work_state.get_ready_tasks()
